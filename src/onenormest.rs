...
use full::slice::{argmax, argsort, max, norm_inf};
use full::Mat;
use rand::Rng;
...
pub fn onenormest<M: Matrix>(a_inv: M, t: Option<usize>, it_max: Option<usize>) -> Result<f64> {
    ...
    let mut x_mat = Mat::ones(n, t, true);
    if t > 1 {
        for i in 1..t {
            // These are technically initial samples, not resamples,
            // so the resampling count is not incremented.
            resample_column(i, &mut x_mat)
        }
        for i in 0..t {
            while column_needs_resampling(i, &x_mat, None) {
                resample_column(i, &mut x_mat);
                // nresamples += 1;
            }
        }
    ...
    let mut est_old = 0.0;
    let mut s_mat = Mat::zeros(n, t, true);
    let mut k = 1;
    let mut ind: Option<Vec<usize>> = None;

    let mut est: f64;
    loop {
        let mut y_mat: Mat<f64> = a_inv.no_trans(&x_mat);
        // nmults += 1;
        let mags = sum_abs_axis0(&y_mat);
        est = max(&mags);
        let best_j = argmax(&mags);
        let ind_best = if est > est_old || k == 2 {
            if k >= 2 {
                Some(ind.as_ref().unwrap()[best_j])
            } else {
                None
            }
            // w = Y.col(best_j);
        ...
        s_mat = sign_round_up(&mut y_mat);
        // del Y

        // (2)
        if every_col_of_x_is_parallel_to_a_col_of_y(&s_mat, &s_old) {
            break;
        }
        if t > 1 {
            ...
            for i in 0..t {
                while column_needs_resampling(i, &s_mat, Some(&s_old)) {
                    resample_column(i, &mut s_mat);
                    // nresamples += 1;
                }
            }
        }
        ...
        // (3)
        let z_mat = a_inv.trans(&s_mat);
        // nmults += 1;
        let h = max_abs_axis1(&z_mat);
        // del Z
        ...
        if let Some(ind_) = &ind {
            for j in 0..t {
                elementary_vector(x_mat.col_slice_mut(j), ind_[j]);
...
fn sign_round_up(x: &Mat<f64>) -> Mat<f64> {
    ...
    // Y /= np.abs(Y);
    let mut y = x.clone();
    y.values_mut().iter_mut().for_each(|v| {
        if *v == 0.0 {
            *v = 1.0;
        } else {
            *v = v.abs();
...
fn max_abs_axis1(x: &Mat<f64>) -> Vec<f64> {
...
fn sum_abs_axis0(x: &Mat<f64>) -> Vec<f64> {
    // block_size = 2**20
    // r = None
    ...
    x.row_iter()
        .map(|row| {
            ...
            rowsum
            // row.iter().map(|xi| xi.abs()).sum()
        })
        .collect()
}

fn elementary_vector(v: &mut [f64], i: usize) {
    // let mut v = vec![0.0; n];
    v.iter_mut().for_each(|w| *w = 0.0);
    v[i] = 1.0;
}

fn vectors_are_parallel(v: &[f64], w: &[f64]) -> bool {
...
fn every_col_of_x_is_parallel_to_a_col_of_y(x: &Mat<f64>, y: &Mat<f64>) -> bool {
    for v in x.col_iter() {
        for w in y.col_iter() {
...
fn column_needs_resampling(i: usize, x: &Mat<f64>, y: Option<&Mat<f64>>) -> bool {
    ...
    let v = x.col_slice(i);
    if (0..i)
        .map(|j| vectors_are_parallel(v, x.col_slice(j)))
        .any(identity)
    {
        return true;
    }
    ...
    if let Some(y) = y {
        if (0..i)
            .map(|j| vectors_are_parallel(v, y.col_slice(j)))
            .any(identity)
        {
            return true;
        }
...
fn resample_column(i: usize, x: &mut Mat<f64>) {
